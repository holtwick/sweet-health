...
import type { ConfigurationProps, DexcomEntry, GlucoseEntry, LatestGlucoseProps } from './_types'
import { Trend } from './_types'
...
export class DexcomClient {
  ...
  constructor({
    ...
    this.host = server === 'us' ? 'share2.dexcom.com' : 'shareous1.dexcom.com'
  }
    ...
    try {
      ...
      return data.map((entry: DexcomEntry) => {
        ...
        if (typeof trend === 'number')
          trend = Trend[trend - 1]
        const ts = extractNumber(entry.WT) as number
        return {
          mmol: mgdlToMmol(entry.Value),
          mgdl: entry.Value,
          timestamp: Math.floor(ts / 1000) * 1000,
  ...
  private apiUrl(resource: string) {
    return `https://${this.host}/ShareWebServices/Services/${resource}`
  }
}
