...
<template>
  <div class="app-events-graph _graph">
    <svg stroke-linecap="round" display="block" stroke-width="2" :viewBox="viewBox">
      <defs>
        <linearGradient id="888" gradientUnits="userSpaceOnUse" x1="0" y1="100%" x2="0" y2="0">
          <stop offset="0" stop-color="var(--state-ok)" />
          <stop :offset="line.high" stop-color="var(--state-ok)" />
          <stop :offset="line.high" stop-color="var(--state-warn)" />
          <stop :offset="line.veryHigh" stop-color="var(--state-warn)" />
          <stop :offset="line.veryHigh" stop-color="var(--state-critical)" />
          <stop offset="1" stop-color="var(--state-critical)" />
        </linearGradient>
      </defs>
      <template v-for="x in line.hours" :key="x">
        <line :x1="x" :x2="x" y1="0" :y2="height" stroke="#eee" stroke-width="1" />
      </template>

      <line :x1="0" :x2="width" :y1="line.highY" :y2="line.highY" stroke="var(--state-warn)" stroke-opacity="0.4" stroke-width="1" />
      <line :x1="0" :x2="width" :y1="line.veryHighY" :y2="line.veryHighY" stroke="var(--state-critical)" stroke-opacity="0.4" stroke-width="1" />
